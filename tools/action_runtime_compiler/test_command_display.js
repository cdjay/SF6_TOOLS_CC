...
function modernText(entry) {
    ...
    const displays = [entry.simple_command && entry.simple_command.display,
        entry.motion_command && entry.motion_command.display].filter(Boolean);
...
const officialSemantics = {
    _meta: { schema: "modernText(xt).v1", character: "Zangief", generated_from: "capcom_official" },
...
assert.strictEqual(modernText(output["1400"]), "中 + 强");
...
assert.strictEqual(modernText(output["1501"]), "6");
assert.strictEqual(modernText(output["1502"]), "4");
assert.strictEqual(modernText(output["1503"]), "N");
...
assert.strictEqual(modernText(output["1511"]), "N");
assert.strictEqual(output["1511"].routes[0].source, "ac_type13_neutral_continuation");
assert.strictEqual(modernText(output["1600"]), "2 + 中 + 强");
assert.strictEqual(modernText(output["1601"]), "8");
assert.strictEqual(output["1601"].routes[0].source, "ac_type20_state_direction");
assert.strictEqual(modernText(output["1602"]), "6");
assert.strictEqual(output["1602"].relation, undefined);
assert.strictEqual(modernText(output["600"]), "弱");
assert.strictEqual(modernText(output["605"]), "AUTO + 强");
assert.strictEqual(modernText(output["606"]), "> 强");
...
assert.strictEqual(modernText(output["652"]), "空中 6 + 强");
assert.strictEqual(modernText(output["653"]), "空中 4 + 强");
...
assert.strictEqual(modernText(output["647"]), "空中 强");
assert.strictEqual(modernText(output["692"]), "空中 2 + 强");
assert.strictEqual(modernText(output["480"]), "DP");
assert.strictEqual(modernText(output["739"]), "DRC");
assert.strictEqual(modernText(output["740"]), "RAW DR");
assert.strictEqual(modernText(output["850"]), "DI");
assert.strictEqual(modernText(output["855"]), "DI");
assert.strictEqual(modernText(output["903"]), "SP/236 + 强");
assert.strictEqual(modernText(output["681"]), "> 中");
assert.strictEqual(modernText(output["985"]), "> 强");
...
assert.strictEqual(modernText(output["904"]), "AUTO + SP/236 + 任意键 + 任意键");
assert.strictEqual(output["975"], undefined);
assert.strictEqual(modernText(output["976"]), "236 + 弱");
assert.strictEqual(output["976"].routes.some(route => route.profile === "supr"), false);
assert.strictEqual(modernText(output["977"]), "AUTO + SP/236 + 任意键 + 任意键");
assert.strictEqual(modernText(output["978"]), "236 + 弱");
...
assert.strictEqual(modernText(output["918"]), "4 + SP/63214 + 任意键");
assert.strictEqual(modernText(output["924"]), "4 + AUTO + SP/63214 + 任意键 + 任意键");
assert.strictEqual(modernText(output["1075"]), "6 + 弱 + 中 + 强");
assert.strictEqual(modernText(output["1076"]), "4 + 中 + 强");
...
assert.strictEqual(modernText(output["1218"]), "2 + SP + 强/720 + 强");
assert.strictEqual(modernText(output["1231"]), "弱 > 弱 > 中 > 强");
assert.strictEqual(modernText(output["1240"]), "[2] + SP + 强/[4]646 + 强");
assert.strictEqual(modernText(output["1302"]), "[4] + SP + 强");
assert.strictEqual(modernText(output["1303"]), "[4] + SP");
assert.strictEqual(modernText(output["1300"]), "[4] + SP/[4]6 + 中");
assert.strictEqual(modernText(output["1301"]),
...
for (const id of [917, 923, 979]) {
    assert.strictEqual(modernText(output[String(id)]), null);
...
for (const [id, display] of Object.entries({ 17: "66", 18: "44", 36: "8", 37: "9", 38: "7", 489: "DP" })) {
    ...
    assert.strictEqual(modernText(entry), display);
...
assert.strictEqual(modernText(output["716"]), "2 + THROW");
...
assert.strictEqual(modernText(output["717"]), "4 + THROW");
...
for (const [id, entry] of Object.entries(output).filter(([key]) => /^\d+$/.test(key))) {
    ...
    for (const route of entry.routes) {
        ...
        commandDisplay.assertValidDisplay(route.display);
    }
}
...
const rebuild = (ac, bcm, compiled) => commandDisplay.buildCommandDisplay(
    ac || actionSource, bcm || catalog, compiled || runtime, {}, { generatedAt: "negative" });
const assertNoModernCommand = entry => assert(entry === undefined || modernText(entry) === null);
...
const staleIdOutput = commandDisplay.buildCommandDisplay({}, semanticCatalog([626, 627, 628]),
    semanticRuntime([626, 627, 628]), {}, { generatedAt: "stale", officialSemantics: semanticInput });
assert.strictEqual(modernText(staleIdOutput["626"]), "AUTO + 弱");
...
const tiedOutput = commandDisplay.buildCommandDisplay({}, semanticCatalog([624, 626]),
...
const rowSemanticOutput = commandDisplay.buildCommandDisplay({}, rowSemanticCatalog, rowSemanticRuntime, {},
    { generatedAt: "semantic-rows", officialSemantics: rowSemanticInput });
assert.strictEqual(modernText(rowSemanticOutput["600"]), "AUTO + 弱");
assert.strictEqual(modernText(rowSemanticOutput["601"]), "弱");
...
const autoAirOutput = commandDisplay.buildCommandDisplay({}, autoAirCatalog, autoAirRuntime, {},
    { generatedAt: "auto-air" });
assert.strictEqual(modernText(autoAirOutput["650"]), "空中 弱");
assert.strictEqual(modernText(autoAirOutput["653"]), "空中 AUTO + 弱");
...
const specialRuntime = { character: "OfficialSpecial", fighter_id: 103,
    ...
    evidence: { ac_derived_commands: [], alias_relations: [] } };
const specialOutput = commandDisplay.buildCommandDisplay({}, specialCatalog, specialRuntime, {},
    { generatedAt: "internal-selector" });
assert.strictEqual(modernText(specialOutput["2000"]), "236 + 弱");
assert.strictEqual(specialOutput["2000"].routes.some(route => route.profile === "supr"), false);
assert.strictEqual(modernText(specialOutput["2001"]),
    "2 + AUTO + SP/236 + 任意键 + 任意键");
assert.strictEqual(modernText(specialOutput["2002"]), "22 + 任意键");
assert.strictEqual(modernText(specialOutput["2003"]), "4 + SP");
assert.strictEqual(modernText(specialOutput["2005"]), "4 + AUTO + SP");
assert.strictEqual(specialOutput._meta.audit.paired_sprt_sp_relation_count, 1);
assert.strictEqual(modernText(specialOutput["2004"]), "22 + 任意键");
assert.strictEqual(modernText(specialOutput["2006"]), "3 + SP");
assert.strictEqual(modernText(specialOutput["2007"]), "3 + AUTO + SP");
assert.strictEqual(modernText(specialOutput["2008"]), "> 空中 任意键");
...
const splitOutput = commandDisplay.buildCommandDisplay({}, splitCatalog, splitRuntime, {},
...
assert.strictEqual(modernText(splitOutput["5001"]), "SP/任意键 + 任意键");
...
const holdRuntime = { character: "HoldEvidence", fighter_id: 104,
    ...
    actions: { "3000": "22+P" },
    evidence: { ac_derived_commands: [], alias_relations: [] } };
const holdOutput = commandDisplay.buildCommandDisplay(makeStrictHoldActionSource(0, -1),
    holdCatalog, holdRuntime, {}, { generatedAt: "type20-hold" });
assert.strictEqual(modernText(holdOutput["3001"]), "> 任意键");
...
const rejectedHold = commandDisplay.buildCommandDisplay(makeStrictHoldActionSource(-1, -1),
...
const phaseRuntime = { character: "ActionPhase", fighter_id: 106,
    ...
    actions: { "3100": "236+MP" },
    evidence: { ac_derived_commands: [], alias_relations: [] } };
const phaseOutput = commandDisplay.buildCommandDisplay(makeType20ActionPhaseSource(true), phaseCatalog,
    phaseRuntime, {}, { generatedAt: "type20-action-phase" });
assert.strictEqual(modernText(phaseOutput["3100"]), "6 + SP");
assert.strictEqual(modernText(phaseOutput["3101"]), "6 + SP");
...
assert.strictEqual(modernText(aliasOutput["979"]), "SP/236 + 强");
assert.strictEqual(modernText(aliasOutput["903"]), "SP/236 + 强");
...
const ambiguousAliasOutput = commandDisplay.buildCommandDisplay(makeAmbiguousType29ActionSource(),
    ambiguousAliasCatalog, ambiguousAliasRuntime, {}, { generatedAt: "ambiguous-type29" });
assert.strictEqual(modernText(ambiguousAliasOutput["4000"]), "236 + 强");
assert.strictEqual(modernText(ambiguousAliasOutput["4002"]), null);
...
assert.strictEqual(modernText(remapped[String(918 + offset)]),
    "4 + SP/63214 + 任意键");
assert.strictEqual(modernText(remapped[String(924 + offset)]),
    "4 + AUTO + SP/63214 + 任意键 + 任意键");

assert.throws(() => commandDisplay.buildCommandDisplay({}, catalog, runtime, {
...
for (const display of ["LP", "4 + HP", "P", "PP", "K", "KK", "4 + + SP", "攻击三つ", "* + SP"]) {
    assert.throws(() => commandDisplay.assertValidDisplay(display), /非法 Modern 显示 token/);
}
...
console.log("Unified command display compiler tests passed.");
