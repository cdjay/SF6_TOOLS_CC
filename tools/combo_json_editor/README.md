...
编辑器会在浏览器本地记住最近一次打开的目录。再次进入页面时，若目录权限仍有效会自动重新载入；否则点击“刷新目录”重新授权即可。刷新会重新扫描目录并尽量保留当前选中的连段，存在未保存修改时会先确认。

“应用表单修改”只把当前表单暂存到浏览器工作副本。当前连段的升级、应用、保存和下载入口集中在环境页右侧“木人与训练菜单”下方；审核多个文件后，可点击页面最顶部的“保存全部修改”，一次性原地写入全部已暂存文件，按钮会显示待保存数量。
...
- 列表排序：默认保持文件名自然顺序，也可通过“按名称排序”按钮按 JSON 的 `_xt_meta.title` 升序或降序排列；排序规则与游戏列表一致（Lua ASCII 小写 + UTF-8 字节顺序），兼容旧 `_wtt_cn_meta.title`，空标题回退使用无扩展名文件名。
