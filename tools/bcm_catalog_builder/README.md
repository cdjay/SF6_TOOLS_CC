...
直接打开 `index.html`，把 `*-bcm-full-*.json` 拖入页面。转换完全在浏览器本地完成，不上传文件。页面可以筛选 Action ID、比较 `norm/easy/sprt/supr`，并导出审查简表：
...
请把临时输出写到仓库外。正式运行时只使用 `xt.command_display.v1` 统一三槽角色表，
不再生成或加载独立的经典 BCM 运行时表。
...
- BCM 提供四套 profile 的生成证据；exceptions 只负责吸收动作、蓄力窗口、前置动作忽略、强制匹配等行为例外，不得包含显示文本。
...
- 审查简表记录原始完整 BCM 文件的 SHA-256，用于确认来源和发现过期转换结果。
- 证据集把角色写成 `Fab` 时，生成器会用 `fighter_id` 转换成 MOD 使用的规范角色名（例如 20 → `EHonda`）；可选命令行参数仍可显式覆盖。
- 最终统一表只使用游戏 dump：AC 提供实际 Action ID 全集和派生关系，BCM 提供实际指令入口；行为例外不会覆盖指令文本。
