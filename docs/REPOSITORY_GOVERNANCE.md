...
| `plugins/reframework-sf6cc-atomic-file.dll` | Active restricted Runtime JSON bridge for cumulative telemetry, combo-feedback outbox publication, and bounded deletion of a currently listed character Combo JSON. Keep it with its native source; telemetry write/probe paths remain fixed, Combo deletion remains confined to one regular JSON directly under `CustomCombos/<character>/`, and it is not a general filesystem API. |
